...
if(isset($_POST['save'])){
	if(!isset($_POST['nama']) || !isset($_POST['username']) || !isset($_POST['password']) || !$_POST['email']){
		...
		$query = "INSERT INTO satpam (nama,username,password,email) VALUES('$nama', '$username', '$password', '$email')";
		$query = "UPDATE satpam SET nama='$nama', username='$username', password='$password', email='$email' WHERE id=$id";
		if (mysqli_query($connect, $query)) {
			$_SESSION['flash'] = "<div class=\"alert alert-success\" role=\"alert\">Data berhasil diubah</div>";
			header("location:".$WEB_CONFIG['base_url']."satpam.php");
		}else{
...
$id = $_GET['id'];
$query = "SELECT * FROM satpam WHERE id = $id";
