<?php 
$query = "SELECT * FROM bo ORDER BY id DESC";
$result = mysqli_query($connect, $query);
?>

<a href="<?= $WEB_CONFIG['base_url'] ?>bo.php?page=add" class="btn btn-primary mb-3">
	...
	<table class="table table-sm">
		...
		<tbody>
			...
			<tr>
				...
				<td><?= $data['email'] ?></td>
				<td>
					<a href="<?= $WEB_CONFIG['base_url'] ?>bo.php?page=update&id=<?= $data['id'] ?>" class="btn btn-success m-1">
...
function alertDelete(idn) {
	$('#deleteButtonModal').attr('href', '<?= $WEB_CONFIG['base_url'] ?>bo.php?page=delete&id='+idn);
