...
if(isset($_POST['save'])){
	if(!isset($_POST['nama']) || !isset($_POST['username']) || !isset($_POST['password']) || !$_POST['email']){
		...
		$query = "INSERT INTO bo (nama,username,password,email) VALUES('$nama', '$username', '$password', '$email')";
		$query = "UPDATE bo SET nama='$nama', username='$username', password='$password', email='$email' WHERE id=$id";
		if (mysqli_query($connect, $query)) {
			$_SESSION['flash'] = "<div class=\"alert alert-success\" role=\"alert\">Data berhasil diubah</div>";
			header("location:".$WEB_CONFIG['base_url']."bo.php");
		}else{
...
$id = $_GET['id'];
$query = "SELECT * FROM bo WHERE id = $id";
$result = mysqli_query($connect, $query);
$data = mysqli_fetch_array($result);

?>

<a href="<?= $WEB_CONFIG['base_url'] ?>bo.php" class="btn btn-warning mb-3">
